...
switch state {
	case 0: 
	{
		...
		if (instance_place(x + hspeed,y,obj_solid) && instance_place(x + hspeed,y - 12,obj_solid)) || (instance_place(x + hspeed,y,obj_freezetimewall) && instance_place(x + hspeed,y - 12,obj_freezetimewall))
		{
			hspeed = -hspeed
			xs = -xs
		}
		...
		if instance_exists(obj_player)
		{
			...
			if y > obj_player.y
			{
				if vspeed > (target_vspeed * -1)
					vspeed -= 0.15
				xs = -1
			}
			else
			{
				if vspeed < target_vspeed
					vspeed += 0.15
				xs = 1
			}
		}
