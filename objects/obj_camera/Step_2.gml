...
target_x = obj_player.x + obj_player.hspeed * 18.5 + (obj_player.facing * 15) + (boost * 50 * obj_player.facing)
target_y = obj_player.y //+ obj_player.vspeed * 5
target_chargescroll = obj_player.key_up * obj_player.onground * -75 * door
...
if pause_state != pausestate.playerpause && !instance_exists(obj_hitstun)
{
	...
	if abs(chargescroll - target_chargescroll) > 1
		chargescroll += (target_chargescroll - chargescroll) / 30
	else
