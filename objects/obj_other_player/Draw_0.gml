...
if name = "thejabcoalt" || name = "AtmosphericSire" || name = "DouchebagCat"
	if hat = 0
		...
		draw_text_outlined(x - (string_width(name) / 2),y - 90,admincol,c_black,name,1,1)
else if name = "Solorveno"
	if hat = 0
		draw_text_outlined(x - (string_width(name) / 2),y - 48,grousecol,c_black,name,1,1)
	else
		draw_text_outlined(x - (string_width(name) / 2),y - 90,grousecol,c_black,name,1,1)
else if hat = 0
	draw_text_outlined(x - (string_width(name) / 2),y - 48,c_white,c_black,name,1,1)
else
	draw_text_outlined(x - (string_width(name) / 2),y - 90,c_white,c_black,name,1,1)
